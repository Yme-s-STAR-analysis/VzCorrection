...
int main(int argc, char** argv){
    ...
    gPad->SetLogz();
    th->Draw("colz");
    c->Print(Form("vz_dist_corr_%s.pdf(", trg));
    c->Print(Form("vz_dist_corr_%s_heatmap.png", trg));
    ...
    for (int ipage=0; ipage<4; ipage++){ 
        ...
        c->Print(Form("vz_dist_corr_%s.pdf", trg));
        c->Print(Form("vz_dist_corr_%s_p%d.png", trg, ipage+1));
    }
    ...
    c->Print(Form("vz_dist_corr_%s.pdf)", trg));
    c->Print(Form("vz_dist_corr_%s_p5.png", trg));
    ...
    TFile* tsave = new TFile(Form("high_end_point_vz_%s_corr.root", trg), "recreate");
    tsave->cd();
    tg->Write();
    hfunc->Write();
    tsave->Close();
